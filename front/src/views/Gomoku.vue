...
function _checkWin(color: string): boolean {
...
function targetClick(i: number, j: number, byAI: boolean = false) {
...
function moveCheck(i: number, j: number, direction: number[], moveCount: number): boolean {
...
function moveColor(i: number, j: number, direction: number[], moveCount: number): string {
	if (!moveCheck(i, j, direction, moveCount)) return 'error';
	let moveI: number = i + moveCount * direction[0];
	let moveJ: number = j + moveCount * direction[1];
	return state.color[moveI][moveJ];
}

// moveCounts 0번이 1칸쪽 1번이 2칸쪽
function checkEndPoint1(i: number, j: number, direction: number[], moveCounts: number[]): boolean {
...
function checkEndPoint2(i: number, j: number, direction: number[], moveCounts: number[]): boolean {
	if (moveColor(i, j, direction, moveCounts[0]) == 'black'
...
// 3x3 금수
function check3Ban(i: number, j: number, direction: number[]): number {
...
// 10 .... 23
function checkEndPoint3(i: number, j: number, direction: number[], moveCounts: number[]): boolean {
	if (moveColor(i, j, direction, moveCounts[0]) == 'black'
...
// 0 .... 1
function checkEndPoint4(i: number, j: number, direction: number[], moveCounts: number[]): boolean {
	if (moveColor(i, j, direction, moveCounts[0]) == 'black'
		|| moveColor(i, j, direction, moveCounts[1]) == 'black'
		) {
		return false;
...
// 4x4 금수
function check4Ban(i: number, j: number, direction: number[]): number {
	// TBBB
...
// 장목 금수
function check6Ban(i: number, j: number, direction: number[]): number {
...
function checkBan(i: number, j: number): boolean {
	let BanCount3: number;
	let BanCount4: number;
	let BanCount6: number;
...
function isBanSearchTarget(i: number, j: number): boolean {
	let limit = 2;
...
async function putClick(byAI: boolean = false) {
	if (state.player == 'none') {
		state.colorSelectVisible = true;
		return;
	}
	...
	if (state.winState != 0 || (state.turn != state.player && !byAI)) {
		...
		console.log(state.winState, state.turn, state.player, byAI);
		console.log(state.winState != 0, state.turn != state.player, !byAI, (state.turn != state.player && !byAI));
	}
